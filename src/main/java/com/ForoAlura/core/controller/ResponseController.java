...
public class ResponseController {
    ...
    @GetMapping
    public ResponseEntity<Page<ResponseReturnDetailedDTO>> readAll(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "2") int size){
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ResponseReturnDetailedDTO> update(@RequestBody @Valid ResponseRegister datosRespuesta, @PathVariable(value = "id") Long id){
        return ResponseEntity.ok(this.responseService.update(id,datosRespuesta));
    }

    @PostMapping
    public ResponseEntity<ResponseReturnDetailedDTO> register(@RequestBody @Valid ResponseRegister responseRegister, UriComponentsBuilder uriComponentsBuilder){
